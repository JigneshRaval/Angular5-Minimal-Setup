...
export class BootstrapGrowlService {
    ...
    public addAlert(message: string, type: BootstrapAlertTypes, dismissable?: boolean): void {
    ...
    private _removeAlert(alert: BootstrapAlert, alertHolder: BootstrapAlert[], alerts: Subject<BootstrapAlert[]>) {
    ...
    private _convertTypeToCssClass(type: BootstrapAlertTypes): string {
        if (type === BootstrapAlertTypes.SUCCESS) {
            return "success";
        } else if (type === BootstrapAlertTypes.INFO) {
            return "info";
        } else if (type === BootstrapAlertTypes.WARNING) {
            return "warning";
        } else if (type === BootstrapAlertTypes.DANGER) {
            return "danger";
        }
    }
